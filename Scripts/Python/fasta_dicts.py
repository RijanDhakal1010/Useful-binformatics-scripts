...
import argparse # this is the package needed to make the commandline argumen

# this is the name of the parser object that will parse the commandline
parser = argparse.ArgumentParser(description="This script will take a directory and convert all the fasta files in directory in pickled dicts, where the name of the sequence is the key and the sequence is the key. The fasta file extension that are read by defaul are .fna, .faa, .fa and .pep. The out files will always have pickle extensions.")
...
def main():
    ...
    if args.loc == '.':
        ...
        os.chdir(args.loc)
        file_names = os.listdir(args.loc)
