...
parser = argparse.ArgumentParser(description="This program is designed to take in the names of a list of columns and only keep those names from a larger csv file with more columns. For example, if we have a csv file with a 100 columns but we only want a csv file with 45 columns then we will have to supply the names of those 100  columns.")
